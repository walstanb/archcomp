    ...
    if request.method == 'POST':
        if request.FILES:
            ...
            if not os.path.exists(file_name):
                ...
                q = Queue(connection=redis.Redis(host=settings.REDIS_HOST, port=settings.REDIS_PORT, db=settings.REDIS_DB), default_timeout=300)
    ...
    if request.method == 'GET':
        try:
            ...
            if status == "success":
                ...
                file_urls = [request.build_absolute_uri(f'/download/{str(uuid)}/{file}') for file in file_list if file.split(".")[-1] == "csv"]
                files = [{'name': file_list[i], 'url': file_urls[i]} for i in range(len(file_list))]
